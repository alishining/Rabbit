...
var sql = {
	...
	student_sport_report : 'select a.student_id, a.student_name, a.sex, b.item_id, b.item, b.record, b.score, b.level from student_info a left outer join report b on a.student_id = b.student_id where b.item_id in (?) and a.sex like ? and a.class_id=? and b.year=? and b.term=? and a.school_id = b.school_id and b.school_id=? and b.student_id is not null order by student_id,item_id',
	sport_item_report_rate : 'select b.item_id, b.item, sum(b.level=\'3\') as three, sum(b.level=\'3\')/count(*)*100 as three_rate, sum(b.level=\'2\') as two, sum(b.level=\'2\')/count(*)*100 as two_rate, sum(b.level=\'1\') as one, sum(b.level=\'1\')/count(*)*100 as one_rate, sum(b.level=\'0\') as zero,sum(b.level=\'0\')/count(*)*100 as zero_rate, count(*) as total from student_info a left outer join report b on a.student_id = b.student_id where b.item_id in (?) and b.year=? and a.class_id like (?) and a.school_id=? and b.term=\'1\' group by item',
	grade_sport_item_rank : 'select a.class_id,sum(b.score)/count(*) as avg,max(cast(b.score as DECIMAL)) as max from student_info a left outer join report b on a.student_id = b.student_id where b.year=? and b.item_id=? and a.class_id like (?) and a.school_id=? group by a.class_id order by sum(b.score)/count(*) desc',
	class_level_chart : 'select b.level, a.sex, a.student_name from student_info a left outer join report b on a.student_id = b.student_id where b.year=? and b.term = \'1\' and a.class_id=? and b.item_id=? and a.school_id=? and b.student_id is not null',
	health_record : 'select distinct a.student_name, a.class_id, a.sex, a.birth, a.student_id, a.nationality, b.year, b.term, b.health_item, b.item, b.item_id, b.record, b.unit, b.score, b.level from student_info a left outer join report b on a.student_id = b.student_id where a.sex like ? and a.class_id=? and b.term=? and b.year=? and b.school_id=? and b.student_id is not null order by student_id',
	...
	mov_student : 'delete from student_info where school_id = ?',
	...
	get_detail_homework : 'select student_name, student_number, sex, score_list from student_info a left outer join training_record b on a.student_id = b.student_id and b.item=? and ds=? where a.school_id=? and a.class_id=?',
