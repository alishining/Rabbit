...
var sql = {
	...
	mod_school : 'update school set fee_status=?, contract=?, protocol_start=?, protocol_end=?, ipad=?, school_addr=?, school_contact=?, phone=?, contract_status=? where id=?',
	get_school :'select a.*, b.name as manager from (select distinct a.*, b.name as proxy_name from school a left outer join proxy b on a.proxy=b.id  where a.province like ? and a.city like ? and a.district like ? and school like ? and a.is_delete=0 order by id desc) a left outer join manager b on a.manager = b.id',
	get_school_from_proxy : 'select a.*, b.name as manager_name from (select a.*, b.name from school a left outer join proxy b on a.proxy =b.id where a.proxy=? and a.is_delete=0) a left outer join manager b on a.manager = b.id',
	...
	get_school_num : 'select count(*) as count from school where school=? and is_delete=0',
	...
	add_manager : 'insert into manager(name,sex,phone,proxy,is_delete,has_work) values(?,?,?,?,?,?)',
	get_manager : 'select * from manager where proxy = ? and is_delete!=2',
	mod_manager_work : 'update manager set has_work=? where id = ?',
	del_manager : 'update manager set is_delete=1 where id = ?',
	mov_manager : 'update manager set is_delete=2 where id = ?',
	...
	get_proxy_list : 'select a.*, b.name as manager from (select  a.*, b.manager as manager, case when b.id is null then count(*)-1 else count(*) end as count from proxy a left outer join school b on a.id=b.proxy and b.id is not null where b.is_delete=0 and a.province like ? and a.city like ? and a.district like ? and a.name like ? group by a.id order by id desc) a left outer join manager b on a.manager = b.id',
	...
	search_school : 'select * from school where is_delete=0 and school like ?',
	...
	//-------------------------------------------------------------------
	school_login     : 'select teacher_name, school_id, school, password, is_root from school_user where account=? and is_delete=0',
	...
	//-------------------------------------------------------------------
	get_book_list : 'select * from book_list order by level',
